...
### Pre-submit phase (`buildPrepareRequest` / `parsePrepareResponse`)

Some vendors cannot accept a task reference inline: the asset must be
registered first (an upload endpoint that returns an identifier), and the task
then references that identifier. A plugin declares this with
`buildPrepareRequest`, which returns one `RequestDescriptor` or an array of
them; returning nothing (or a descriptor with an empty `url`) skips the phase.

```js
export function buildPrepareRequest(ctx) {
  if (!ctx.files.length) return null
  return {
    url: ctx.baseUrl + "/materials",
    method: "POST",
    bodyType: "multipart",
    parts: [
      { name: "material_id", value: "pg-" + ctx.publicTaskId },
      { name: "file", fileRef: ctx.files[0].ref, filename: "reference_1.png" },
    ],
  }
}
...
The host sends each descriptor (reusing the same body encoder as the submit
request, so multipart uploads and inlined file placeholders behave identically),
then calls `parsePrepareResponse` with the decoded responses
(`{statusCode, headers, body}` in request order). The returned value is exposed
to `buildSubmitRequest` as `ctx.prepared`. Both hooks must be exported together;
a prepare failure aborts the submission before any upstream task is created, and
